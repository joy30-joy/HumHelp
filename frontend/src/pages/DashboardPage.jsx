...
export default function DashboardPage() {
  ...
  const handleRequestCreated = (r) => setRequests(reqs => [r, ...reqs]);
  const handleStatusUpdate = (requestId, status) => setRequests(reqs => reqs.map(r => r._id === requestId ? { ...r, status } : r));
          ...
          <Tab.Panel>
            {loading ? (
              ...
              <div className="grid gap-4 mt-4">
                {requests
                  .filter(r =>
                    (r.recipient._id === user._id || r.requester._id === user._id) && r.status !== "pending"
                  )
                  .map(request => (
                    <RequestCard
                      key={request._id}
